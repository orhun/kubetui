...
use crate::kube::apis::networking::gateway::v1::{
    AllowedRoutes, FromNamespaces, Gateway, HTTPRoute, ParentReference, RouteNamespaces,
};

use super::{label_selector_to_query, ExtractNamespace as _};
...
pub async fn discover_httproutes(
    ...
    for listener in &gateway.spec.listeners {
        ...
        if let Some(RouteNamespaces { from, selector }) = namespaces {
            let Some(FromNamespaces(from)) = from else {
                continue;
            };

            let httproutes = match from.as_str() {
                "All" => {
...
async fn discover_httproute_for_all(
    ...
    let httproutes = api.list(&Default::default()).await?;
        ...
        refs.iter().any(
            |ParentReference {
                ...
                compare_parent_ref_group(group.as_ref().map(|g| g.as_str()), Gateway::GROUP)
                    ...
                    && compare_parent_ref_name(name, gateway_name)
